...
if(preg_match('/^(s?f|ht)tps?:\/\/[^\/]+/i', (string)$_GET['image'])) {

	$imageUrl = end(explode('image=', $_SERVER['REQUEST_URI']));
	$imageMD5 = md5($imageUrl);
	if(!file_exists(CACHE_DIR . md5($imageUrl))) {

		$imageData = @file_get_contents($imageUrl);
		file_put_contents(CACHE_DIR .$imageMD5, (($imageData) ? $imageData : file_get_contents('images/placeholder.jpg')));
		
	}

	$image = CACHE_DIR . md5($imageUrl);
...
if (!isset($_GET['image']) || $_GET['image'] == '') $_GET['image'] = $image = 'images/placeholder.jpg';
...
{
	$_GET['image'] = $image = 'placeholder.jpg';
	$size	= @GetImageSize($docRoot . $image);
	$mime	= $size['mime'];
}
